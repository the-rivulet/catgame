...
function updateElement() {
	...
	const catlimtemp = new Decimal(1000).pow(data.catLimit).times(new Decimal(0.85).pow(data.lessSpace));

	// Store repeating elements from gel into an object


	// Update buyallbutton text content
	elements.buyallbutton.textContent = data.catLimit.gte(6) ? "Buy All" : "Locked";

	// Update hcbutton text content
	if (data.cats.gte(new Decimal(1e3).times(new Decimal(4e2).pow(data.hyperCats)))) {
		elements.hcbutton.textContent = "Hyper Cat Reset";
	} else {
		elements.hcbutton.textContent = "You need " + formatForH(new Decimal(1e3).times(new Decimal(4e2).pow(data.hyperCats))) + " Cats to (HC)Reset";
	}

	// Update hvcbutton text content
	if (data.voidCats.gte(new Decimal(1e5).times(new Decimal(2.25).pow(new Decimal(0.93).times(data.voidhypercats))))) {
		elements.hvcbutton.textContent = "Hyper Void-Cat Reset";
	} else {
		elements.hvcbutton.textContent = "You need " + formatForH(new Decimal(1e5).times(new Decimal(2.25).pow(new Decimal(0.93).times(data.voidhypercats)))) + " Void Cats to (HVC)Reset";
	}
	...
	if (data.cats.gte(1)) {
		...
		elements.catbarpercent.textContent = formatForH(data.cats.log(10).divide(catlimtemp.log(10)).times(100)) + "%";
	} else {
	...
	elements.basecats.textContent = formatForH(data.catSummoners.pow(data.betterMagic.divide(60).plus(1)));
	elements.catmulti.textContent = formatForH(new Decimal(1.75).pow(data.catFood));
	elements.cscost.textContent = formatForH(new Decimal(10).times(new Decimal(1.75).pow(data.catSummoners)));
	elements.cfcost.textContent = formatForH(new Decimal(25).times(new Decimal(2.05).pow(data.catFood)));
	elements.catlimitnum.textContent = formatForZ(data.catLimit.minus(1));
	elements.maxccs.textContent = formatForZ(data.catLimit);
	elements.ccs.textContent = formatForZ(data.catLimit.minus(data.spentCatCoins));
	elements.ls.textContent = formatForZ(data.lessSpace);
	elements.bm.textContent = formatForZ(data.betterMagic);
	elements.lscost.textContent = formatForZ(new Decimal(1.15).pow(data.lessSpace).floor());
	elements.bmcost.textContent = formatForZ(new Decimal(1.25).pow(data.betterMagic).floor());
	elements.lsboost.textContent = formatForH(new Decimal(0.85).pow(data.lessSpace).times(100));
	elements.bmboost.textContent = formatForH(data.betterMagic.divide(60).plus(1));
	elements.catssacrificed.textContent = formatForH(data.catGodSacrificed);
	elements.sacrificedboost.textContent = formatForH(((data.catGodSacrificed.plus(1)).logarithm(new Decimal(5).minus(voidUpgradeBoosts(1))).plus(1)).pow(1.01));
	elements.hc.textContent = formatForZ(data.hyperCats);
	elements.hcboost.textContent = formatForH(data.hyperCats.plus(1).pow(voidUpgradeBoosts(2)));
	elements.timeplayed.textContent = formatTime(new Decimal(Date.now()).minus(data.startPlaying).divide(1000));
	...
	elements.voidg.textContent = formatForH(data.energyCats.log(10).times(new Decimal(1.375).pow(data.voidhypercats)));
	elements.hypervoidcats.textContent = formatForZ(data.voidhypercats);
	elements.hypervoidcatsboost.textContent = formatForH(new Decimal(1.375).pow(data.voidhypercats));
	elements.voidb.textContent = formatForH(!isNaN(data.cats.log(10).mag) ? data.voidCats.plus(1).log(3).times((catlimtemp.log(10).divide(data.cats.plus(1).log(10)))).plus(1) : 1);
	...
	const percent_to_cl = isNaN(data.cats.log(10).divide(catlimtemp.log(10)).mag) ? Decimal.dZero : data.cats.log(10).divide(catlimtemp.log(10));
	elements.voidpower.textContent = formatForH(data.voidCats.pow(0.6).times(Decimal.dOne.minus(percent_to_cl)));
	elements.catlimitboost.textContent = data.voidup3.lt(1) ? "" : "Boost: *" + format((((new Decimal(1.05).plus(voidUpgradeBoosts(3))).pow(data.catLimit.minus(1)))), 3) + " More Cats";
...
for (let i = 0; i < 4; i++) {
	...
	if (data.energyCats.gte(emvalues[i])) {
		...
		emElement.textContent = formatForH(new Decimal(emvalues[i]).minus(data.energyCats)) + " ECs left (" + formatForH(emvalues[i]) + " Required)";
	}
}
...
function buyCatUpgrade(itemIndex: number, catValue: Decimal, base: number) {
	if (itemIndex === 1 && data.cats.gte(new Decimal(10).times(Decimal.pow(base, catValue)))) {
		data.cats = data.cats.minus(new Decimal(10).times(Decimal.pow(base, catValue)));
		catValue.plus(1);
	}

	if (itemIndex === 2 && data.cats.gte(new Decimal(25).times(Decimal.pow(base, catValue)))) {
...
function buyUnlockUpgrade(itemIndex: number, unlockValue: Decimal, threshold: number) {
	if (itemIndex === 1 && unlockValue.eq(0) && data.catGodSacrificed.gte(threshold)) {
...
function voidUpgradeBoosts(x: Decimal | number) {
	const catlimtemp = (new Decimal(1000).pow(data.catLimit)).times((new Decimal(0.85).pow(data.lessSpace)))
	const percent_to_cl = isNaN(data.cats.log(10).divide(catlimtemp.log(10)).mag) ? Decimal.dZero :
		data.cats.log(10).divide(catlimtemp.log(10))
	const voidenergy = data.voidCats.pow(0.6).times(Decimal.dOne.minus(percent_to_cl))
	if (x == 1) {
...
function updateTabs() {
	...
	gel('catlimbutton').style.display = data.cats.gte(999) || data.catLimit.gte(2) ? 'inline' : 'none';
...
function vHyperCatsReset(multicat: number) {
	...
	if (mulcat.gte(1)) { }
		...
		if (data.voidCats.gte(new Decimal(1e5).times(new Decimal(2.25).pow(new Decimal(0.93).times(data.voidhypercats))))) {
			data.voidCats = new Decimal(1e5).times(new Decimal(2.25).pow(new Decimal(0.93).times(data.voidhypercats)))
			data.voidhypercats = data.voidhypercats.plus(1)
		}
	}
}
function catLimitReset(force = 0) {
	let catlimtemp = (new Decimal(1000).pow(data.catLimit)).times((new Decimal(0.85).pow(data.lessSpace)))
	if (data.cats.greaterThanOrEqualTo(catlimtemp) || force == 1) {
		data.cats = new Decimal(11)
		if (force == 0) { data.catLimit = data.catLimit.plus(1) }
		data.catSummoners = Decimal.dZero
		data.catFood = Decimal.dZero
...
function buyAll() {
	if (data.catLimit.gte(6)) {
		while (data.cats.greaterThanOrEqualTo(new Decimal(10).times(new Decimal(1.75).pow(data.catSummoners))) || data.cats.greaterThanOrEqualTo(new Decimal(25).times(new Decimal(2.05).pow(data.catFood)))) {
...
setInterval(() => {
	const catlimtemp = (new Decimal(1000).pow(data.catLimit)).times((new Decimal(0.85).pow(data.lessSpace)))
	if (data.cats.gte(data.highCats)) { data.highCats = data.cats }
	...
	const base_cps = (data.catSummoners.pow(data.betterMagic.divide(60).plus(1))).times(new Decimal(1.75).pow(data.catFood))

	//1.05^CL
	const catlimit_boost = ((new Decimal(1.05).plus(voidUpgradeBoosts(3))).pow(data.catLimit.minus(1)))

	//(log5(cg.SA+1)+1)^1.01
	const catgod_boost = ((data.catGodSacrificed.plus(1)).logarithm(new Decimal(5).minus(voidUpgradeBoosts(1))).plus(1)).pow(1.01)
	...
	const void_boost = !Decimal.isNaN(data.voidCats.plus(1).log(3).times((catlimtemp.log(10).divide(data.cats.plus(1).log(10)))).plus(1)) && data.voidCats.plus(1).log(3).times((catlimtemp.log(10).divide(data.cats.plus(1).log(10)))).plus(1).gte(1) ? data.voidCats.plus(1).log(3).times((catlimtemp.log(10).divide(data.cats.plus(1).log(10)))).plus(1) : 1

	catsPerSecond = base_cps.times(catlimit_boost).times(catgod_boost).times(hypercat_boost).times(void_boost)

	updateElement()

	if (data.catLimit.greaterThanOrEqualTo(8) && !isNaN(data.cats.log(10).mag) && !data.voidGain.gte(1)) { data.energyCats = data.energyCats.plus(data.cats.log(10).times(dt)) }

	if (data.catLimit.greaterThanOrEqualTo(11) && !isNaN(data.energyCats.log(10).mag) && data.voidGain.gte(1)) {
		data.voidCats = data.voidCats.plus((data.energyCats.log(10).times(new Decimal(1.375).pow(data.voidhypercats)
		)).mul(dt))
	}

	data.cats = data.cats.plus(catsPerSecond.mul(dt))

	if (data.cats.greaterThanOrEqualTo(catlimtemp)) { data.cats = catlimtemp }
	...
	if (currentTab == "dogsdogsdogsdogs") { data.cats = Decimal.dZero }
	...
	elements.debug.mspt.textContent = `MSPT: ${dt}ms`;
}, 1000 / 100)
