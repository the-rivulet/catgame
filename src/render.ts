import { calculateVoidBoost, currentTab, data, energyMilestone, loaftime, newsfile, voidUpgradeBoosts } from './data';
// import { getCatSize } from './utils';
import Decimal from 'break_eternity.js';
import { gel, getCatSize } from './utils';
import { format, formatForH, formatForZ, formatTime } from './format';

export const catsPerSecond = { _: Decimal.dZero };
export const timeSinceLastNewsMessage = { _: Decimal.dZero };

export function updateElement(dt: number) {
    // Update the element with various data calculations
    let catlimtemp = new Decimal(1000).pow(data.voidCats.plus(1).log(3).times((data.cats.plus(1).log(10).divide(data.cats.plus(1).log(10))).plus(1))) || 1;
    ...
    if (data.cats.gte(1)) {
        ...
        if (catlimbarValue >= 0.347130895052) catlimbarValue = 0.011;

        gel<HTMLProgressElement>("catlimbar").value = catlimbarValue;
        ...
        gel<HTMLProgressElement>("catlimbar").value = 0;
