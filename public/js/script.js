...
const elements = {
    ...
    cfas: gel("cfas"),
    las: gel("las"),
    voidhypercatstuff: gel("voidhypercatstuff"),
    secretpasses: gel("secretpasses"),
};
...
function formatTime(s) {
    ...
    if (s.gte(1 / 0))
        ...
        return (formatWhole(s.div(60).floor()) + "m " + format(s.toNumber() % 60) + "s");
    else if (s.lt(86400))
        return (formatWhole(s.div(3600).floor()) +
            "h " +
            format(s.div(60).toNumber() % 60) +
            "m");
    else if (s.lt(31536000))
        return (formatWhole(s.div(84600).floor()) +
            "d " +
            formatWhole(s.div(3600).toNumber() % 24) +
            "h");
    else if (s.lt(31536000000))
        return (formatWhole(s.div(31536000).floor()) +
            "y " +
            formatWhole(s.div(84600).toNumber() % 365) +
            "d");
    else
        return formatWhole(s.div(31536000)) + "y";
}
...
const formatForH = (x) => format(x);
const formatForZ = (x) => format(x, 0, true);
const data = {
...
function exportSave() {
    gel("savedata").value = btoa(JSON.stringify(data));
}
function importSave() {
    const data = JSON.parse(atob(gel("savedata").value));
    for (const i in data)
        data[i] = data[i];
    save();
    location.reload();
}
function getCatSize(cats) {
...
function updateElement() {
    ...
    if (data.voidCats.gte(new Decimal(1e5).times(new Decimal(2.25).pow(new Decimal(0.93).times(data.voidhypercats))))) {
        ...
        elements.hvcbutton.textContent =
            "You need " +
                formatForH(new Decimal(1e5).times(new Decimal(2.25).pow(new Decimal(0.93).times(data.voidhypercats)))) +
    ...
    elements.sacrificedboost.textContent = formatForH(data.catGodSacrificed
        .plus(1)
    ...
    elements.voidb.textContent = formatForH(!isNaN(data.cats.log(10).mag)
        ? data.voidCats
            .plus(1)
            .log(3)
    ...
    elements.catlimitboost.textContent = data.voidup3.lt(1)
        ? ""
        : "Boost: *" +
            format(new Decimal(1.05)
                .plus(voidUpgradeBoosts(3))
                .pow(data.catLimit.minus(1)), 3) +
            " More Cats";
...
function buyItem(itemIndex, tabIndex) {
    if (tabIndex === 1) {
...
export function buyAll() {
    if (data.catLimit.gte(6)) {
        while (data.cats.greaterThanOrEqualTo(new Decimal(10).times(new Decimal(1.75).pow(data.catSummoners))) ||
            data.cats.greaterThanOrEqualTo(new Decimal(25).times(new Decimal(2.05).pow(data.catFood)))) {
            buyItem(2, 1);
            buyItem(1, 1);
        }
    }
...
setInterval(() => {
    ...
    const void_boost = !Decimal.isNaN(data.voidCats
        .plus(1)
        .log(3)
        .times(catlimtemp.log(10).divide(data.cats.plus(1).log(10)))
        .plus(1)) &&
    ...
    if (data.catLimit.greaterThanOrEqualTo(11) &&
